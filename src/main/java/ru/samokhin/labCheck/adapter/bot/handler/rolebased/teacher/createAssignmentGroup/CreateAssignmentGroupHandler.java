package ru.samokhin.labCheck.adapter.bot.handler.rolebased.teacher.createAssignmentGroup;
...
import ru.samokhin.labCheck.adapter.bot.service.user.teacher.createAssignmentGroup.CreateAssignmentGroupService;
import ru.samokhin.labCheck.app.api.assignmentGroup.CreateAssignmentGroupInbound;

@Service
@RequiredArgsConstructor
@Slf4j
public class CreateAssignmentGroupHandler implements ProcessHandler {
    private final MessageSender messageSender;
    private final TeacherService teacherService;
    private final CreateAssignmentGroupService createAssignmentGroupService;
    private final CreateAssignmentGroupInbound createAssignmentGroupInbound;
    ...
    public void handleNonCommandUpdate(AbsSender absSender, Message message) {
        ...
        if (!createAssignmentGroupService.exists(tgChatId)) {
            StatusData statusData = createAssignmentGroupService.startCreatingAssignmentGroup(tgChatId);
            ...
            lastMessageForKbDeleting = messageSender.send(tgChatId, "Введите название обучающейся группы:", absSender,
                    inlineKeyboardFactory.createSingleButtonKeyboard("В меню", "В меню"));
            return;
        }
        ...
        boolean isCreated = createAssignmentGroupInbound.execute(messageText);
        if (!isCreated) {
            inlineKeyboardFactory.removeInlineKeyboard(absSender, lastMessageForKbDeleting);
        ...
        createAssignmentGroupService.removeAssignmentGroupData(tgChatId);
